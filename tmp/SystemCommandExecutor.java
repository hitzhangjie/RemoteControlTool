...
public class SystemCommandExecutor {
  ...
  public int executeCommand() throws IOException, InterruptedException {
    ...
    try {
	  ...
	  // input stream (such as when invoking the 'sudo' command, and it
	  // prompts you for a password).
      OutputStream stdOutput = process.getOutputStream();
      
	  // i'm currently doing these on a separate line here in case i need to
	  // set them to null to get the threads to stop.  see
	  // http://java.sun.com/j2se/1.5.0/docs/guide/misc/threadPrimitiveDeprecation.html
      InputStream inputStream = process.getInputStream();
      InputStream errorStream = process.getErrorStream();

	  // these need to run as java threads to get the standard output and
	  // error from the command.  the inputstream handler gets a reference to
	  // our stdOutput in case we need to write something to it, such as with
	  // the sudo command
      inputStreamHandler = 
